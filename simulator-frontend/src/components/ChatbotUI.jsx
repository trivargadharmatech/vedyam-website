...
function ChatbotUI() {
  ...
  const isFromWebsite = searchParams.get('from') === 'website';
  ...
  // Handle auto-send from Dashboard's Quick Ask or Website Redirect
  useEffect(() => {
    let queryToRun = null;
    let initialMessages = [];

    // 1. Check for Website Redirect (from=website)
    const urlParams = new URLSearchParams(window.location.search);
    const fromWebsite = urlParams.get('from') === 'website';
    const queryParam = urlParams.get('query');
    const historyParam = urlParams.get('history');

    if (fromWebsite && queryParam && !isChatLoading) {
      queryToRun = decodeURIComponent(queryParam);
      if (historyParam) {
        try {
          const parsedHistory = JSON.parse(decodeURIComponent(historyParam));
          initialMessages = parsedHistory.map(m => ({
            role: m.role,
            content: m.text
          }));
      ...
      // Remove query and history from URL to prevent re-triggering on refresh
    ...
    // 2. Check for Dashboard Quick Ask
    if (location.state?.initialQuery && !isChatLoading) {
      queryToRun = location.state.initialQuery;
      navigate(location.pathname, { replace: true, state: {} });
    }

    if (queryToRun) {
      setTimeout(() => {
        // Pass the initialMessages if we have them so handleChatSend uses them instead of empty state
        handleChatSendWithHistory(queryToRun, initialMessages);
      }, 300);
    }
  }, []); // Empty dependency array to run only once on mount

  // A helper function to handle chat send with specific pre-existing history
  const handleChatSendWithHistory = async (userMessage, existingHistory) => {
    if (!userMessage.trim()) return;
    
    setInputValue('');
    const newMessages = [...existingHistory, { role: 'user', content: userMessage }];
  ...
  const handleChatSend = async (directMessage = null) => {
    const userMessage = typeof directMessage === 'string' ? directMessage : inputValue;
      ...
      <div className="dashboard-layout">
        ...
        <div className="mobile-header">
          {isFromWebsite ? (
            ...
            <Link to="/" className="icon-btn" title="Back to Simulator">
              <ArrowLeft size={24} />
            </Link>
          )}
          {!isFromWebsite && (
        ...
        <aside className={`sidebar glass-panel ${!sidebarOpen ? 'collapsed' : ''} ${sidebarOpen ? 'mobile-open' : ''}`}>
          <div className="sidebar-header" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '10px' }}>
            ...
            {isFromWebsite ? (
              ...
              <Link to="/" className="nav-btn" style={{ textDecoration: 'none', padding: '8px 12px', background: 'rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '0.85rem' }}>
        ...
        <main className="main-content">
          ...
          <div className="floating-sidebar-btn" style={{ display: 'none' }}></div>
