...
class KnowledgeExplorer:
    ...
    def stream_explore(self, topic: str, path: list = None):
        """
        Stream the exploration answer, followed by suggestions.
        Yields:
            {"type": "token", "content": "..."}
...
Begin your explanation of '{topic}':"""
        ...
        try:
            response_stream = self.chatbot._call_llm_with_prompt(prompt, stream=True)
            full_answer = ""
            ...
            suggestions = self._generate_suggestions(topic, full_answer, path_context)
            yield {"type": "suggestions", "content": [str(s) for s in suggestions]}

            full_path = " → ".join(path + [topic])
            self._prefetch_async(suggestions, full_path)
            
        except Exception as e:
            logging.error(f"Error in stream_explore for '{topic}': {e}")
            yield {"type": "error", "content": "Failed to connect to the knowledge explorer."}
