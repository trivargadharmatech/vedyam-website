...
def is_comparison_query(query: str) -> bool:
    """
    Detects comparison/versus queries involving multiple entities.
    Triggers when the user asks to compare, contrast, or rank topics.
    """
    q = query.lower()
    # Explicit comparison keywords
    compare_keywords = [
        "vs", "versus", "difference between", "differences between",
        "compare", "comparison", "contrast", "which is", "who is supreme",
        "who is greater", "who is higher", "who is more", "better",
        "same as", "similar to", "similarities", "which one",
        "rank", "ranking", "hierarchy"
    ]
    if any(k in q for k in compare_keywords):
        # Must involve 2+ subjects (look for multiple spiritual entities)
        entities = [
            "shiva", "vishnu", "brahma", "krishna", "rama", "durga",
            "saraswati", "lakshmi", "ganesh", "hanuman", "indra",
            "dharma", "karma", "moksha", "yoga", "atman", "brahman",
            "bhakti", "jnana", "raja", "vedas", "upanishads", "gita",
            "mahabharata", "ramayana", "arjuna", "sita"
        ]
        hits = sum(1 for e in entities if e in q)
        return hits >= 2
    return False
...
def format_chat_history(chat_history: list) -> str:
    ...
    for user_query, bot_answer in chat_history:
        formatted_history += f"User: {user_query or ''}\n"
        formatted_history += f"Bot: {bot_answer or ''}\n"
...
class Chatbot:
    ...
    def _call_llm_with_retry(self, content: str, temperature: float = 0.2, stream: bool = False):
    ...
    def _call_llm_with_prompt(self, prompt: str, temperature: float = 0.4, stream: bool = False):
    ...
    def _rewrite_as_standalone(self, query: str, history: list) -> str:
        ...
        history_text = "\n".join(
            [f"User: {h[0] or ''}\nBot: {(h[1] or '')[:200]}" for h in history[-3:]]
        )
    ...
    def _is_context_relevant(self, query: str, docs) -> bool:
        ...
        Improved to support entity-based queries and fuzzy matches for typos.
        """
        import difflib
        stopwords = {
        ...
        for doc in docs[:3]: # check top documents
            ...
            for word in query_words:
                ...
                # 2. Fuzzy match against individual words in the document
                close_matches = difflib.get_close_matches(word, content_words, n=1, cutoff=0.8)
                if close_matches:
                    logging.info(f"Fuzzy match found: '{word}' matched with '{close_matches[0]}' in document.")
                    return True
    ...
    def _construct_prompt(self, query: str, context: str) -> str:
        ...
        # ── Comparison / Versus (highest priority) ────────────────────────────
        if is_comparison_query(query):
            return f"""You are ShastraBot, a wise spiritual guide knowledgeable in Hindu scriptures.

The user is asking a comparison question. Use the Context below as your primary source.

FORMATTING RULES — follow exactly in this order:

1. INTRODUCTION (2-3 sentences): Give a warm, natural opening that frames the comparison.
   - Bold the name of each subject being compared on first mention.

2. INDIVIDUAL OVERVIEWS: For each subject, write 2-3 sentences explaining who/what they are.
   - Use ## Heading for each subject's name.

3. COMPARISON TABLE: Always include a rich markdown comparison table.
   - Use this exact format:

| Aspect | [Subject 1] | [Subject 2] | [Subject 3 if any] |
|--------|-------------|-------------|---------------------|
| Role | ... | ... | ... |
| Attributes | ... | ... | ... |

   - Include 6-10 meaningful rows covering: Role/Function, Key Attributes, Associated Scripture, Cosmic Domain, Symbolism, Devotional Path, Notable Quality, Relationship to Brahman, etc.
   - Make each cell rich and specific — never write just "Yes" or "No" or leave it vague.
   - Bold the most distinctive value in each row if one stands out.

4. CONCLUSION (2-3 sentences): A thoughtful synthesis — what the scriptures say about their relationship or hierarchy.

NEVER say "according to the context". Speak naturally like a teacher.
...
Answer:"""
            ...
            - BOLD RULE: Bold the key term being taught in the first sentence where it is defined (e.g. "**Karma** means..."). Within each step, bold at most one pivotal word or phrase. Do not bold everything — use it only where it genuinely highlights the core insight.
...
Use the Context below as your primary source. Speak naturally and warmly like a teacher.

FORMATTING RULES — follow exactly:
- Use markdown section headings with ## (e.g. ## Background & Origins)
- BOLD RULE: In your opening 2-3 sentence introduction, bold the person's name where it captures the core one-line answer (e.g. "**Krishna** is the Supreme Being..."). Within sections, bold only the most pivotal word or phrase per bullet/sentence — sparingly, not every occurrence.
- Start with a 2-3 sentence warm introduction (no heading for this part)
- Then use these sections with ## headings:
  ## Background & Origins
  ## Their Story
  ## Key Qualities
  ## Teachings & Legacy
  ## Why They Matter
- Use bullet points (- item) for lists within sections
...
FORMATTING RULES — follow exactly:
- Use markdown section headings with ## (e.g. ## Meaning)
- BOLD RULE: In your opening introduction, bold the concept name and the single phrase that best summarizes its core meaning (e.g. "**Karma** is the law of **cause and effect**..."). Within sections, bold only the most important term or idea per point — use bold sparingly so it stays impactful.
- Start with a 2-3 sentence introduction capturing the essence (no heading for this part)
- Then use these sections with ## headings:
  ## Meaning
  ## Origins
  ## Core Principles
...
- Speak directly and clearly like a knowledgeable teacher
- BOLD RULE: Bold the single most important word or phrase that directly answers the question (e.g. if asked about Krishna, bold "**Krishna**" in the sentence that defines who He is). Use bold on at most 1-2 words/phrases total.
...
Answer:"""
    ...
    def process_query(self, query: str, session_history: list = None) -> tuple[str, set[str]]:
        ...
        try:
           ...
           # 70B model is self-grounding — no verifier needed
            ...
            # Moderate confidence — LLM handles tone naturally, no prefix needed
    ...
    def stream_query(self, query: str, session_history: list = None):
        """
        Processes a user query and yields the answer as a stream, followed by the sources.
        Yields:
            dict: {"type": "token", "content": "..."} or {"type": "sources", "content": [...]} or {"type": "error", "content": "..."}
        """
        sources = set()
        full_answer = ""
        try:
            query = sanitize_query(query)
            logging.info(f"User query (stream): {query}")

            active_history = session_history if session_history is not None else self.chat_history
            query = self._rewrite_as_standalone(query, active_history)

            if self._is_ambiguous_followup(query):
                yield {"type": "token", "content": "Your question seems to refer to something mentioned earlier. Could you please clarify who or what you are referring to?"}
                yield {"type": "sources", "content": []}
                return

            docs = self._retrieve_docs(query)
            if not docs:
                yield {"type": "token", "content": "This question is not covered in the available data."}
                yield {"type": "sources", "content": []}
                return

            if not self._is_domain_relevant(query, docs):
                yield {"type": "token", "content": "This question appears to be outside the scope of the available spiritual texts."}
                yield {"type": "sources", "content": []}
                return

            if not self._is_context_relevant(query, docs):
                yield {"type": "token", "content": "I could not find a clear answer in the available texts. Please try rephrasing or asking in more detail."}
                yield {"type": "sources", "content": []}
                return
            ...
            response = self._call_llm_with_retry(prompt, temperature=0.4, stream=True)
            ...
            if session_history is None:
                self.chat_history.append((query, full_answer))
                if len(self.chat_history) > HISTORY_MAX_TURNS:
                    self.chat_history.pop(0)

            yield {"type": "sources", "content": list(sources)}

        except ValueError as e:
            yield {"type": "error", "content": f"⚠️ Invalid input: {str(e)}"}
        except Exception as e:
            logging.error(f"Error streaming query '{query}': {e}", exc_info=True)
