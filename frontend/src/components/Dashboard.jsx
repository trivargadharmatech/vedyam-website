...
import { Download, ArrowLeft, Play, LayoutDashboard, ChevronLeft, Menu } from 'lucide-react';
...
const Dashboard = ({ user }) => {
  ...
  const handleSelectTopic = async (topic) => {
    ...
    try {
      ...
      const rRes = await fetch(`${API_BASE_URL}/api/media`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ topic: topic.title })
      });
  ...
  return (
    <div className="dashboard-container">
      ...
      <main className="simulator-container">
        ...
        <div className="chatbot-banner animate-on-scroll visible" style={{ 
          ...
          <div>
            <h3 style={{ fontSize: '1.2rem', marginBottom: '0.25rem', color: 'var(--text-main)' }}>New General Chatbot</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Try our new advanced, general-purpose Vedyam Chatbot hosted on Hugging Face.</p>
          </div>
          <a 
            href="https://huggingface.co/spaces/vijayyh/VedyamChatBot1.0.0" 
            target="_blank" 
